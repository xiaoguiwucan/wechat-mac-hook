...
LOG_FILE="$AGENT_HOME/logs/voice-transcript-sidecar.log"
...
if [[ -z "$WECHAT_PID" ]]; then
  WECHAT_PID="$("$ROOT_DIR/scripts/find_wechat_pid.sh" | head -1 || true)"
fi
if [[ -z "$WECHAT_PID" ]]; then
...
if [[ "$ACTUAL_EXE" != "$(/usr/bin/python3 -c 'import os,sys; print(os.path.realpath(sys.argv[1]))' "$EXE")" ]]; then
  echo "拒绝附加：PID=$WECHAT_PID 不是当前微信" >&2
  exit 3
fi

# The built-in voice transcript is rendered by WeChatAppEx on recent macOS
# clients. It is a direct child of WeChat, so observing it preserves the same
# process boundary and only observes children of the verified /Applications/WeChat.app PID.
...
PY
  ...
  if [[ "$ACTUAL_UI_EXE" != "$(/usr/bin/python3 -c 'import os,sys; print(os.path.realpath(sys.argv[1]))' "$UI_EXE")" ]]; then
    echo "忽略非当前微信 UI 子进程 PID=$UI_PID" >&2
    UI_PID=""
  fi
fi
...
PY
  ...
  done < <(ps -axo pid=,ppid=,command= | awk -v parent="$UI_PID" '$2 == parent && /WeChatAppEx Helper \(Renderer\).*com\.tencent\.xinWeChat/ { print $1 }')
fi
...
: > "$LOG_FILE"
WECHAT_UI_PID="$UI_PID" WECHAT_RENDERER_PIDS="$RENDERER_PIDS" WECHAT_OCR_BIN="$OCR_BIN" /usr/bin/python3 - "$SIDECAR_BIN" "$WECHAT_PID" "$LOG_FILE" "$PID_FILE" <<'PY'
import os
import subprocess
import sys
...
ui_pid = os.environ.get('WECHAT_UI_PID', '')
if ui_pid:
    cmd.append(f'-ui-pid={ui_pid}')
renderer_pids = os.environ.get('WECHAT_RENDERER_PIDS', '')
if renderer_pids:
    cmd.append(f'-renderer-pids={renderer_pids}')
ocr_bin = os.environ.get('WECHAT_OCR_BIN', '')
if ocr_bin:
...
for _ in {1..20}; do
  ...
  if [[ -n "$SIDECAR_PID" ]] && kill -0 "$SIDECAR_PID" 2>/dev/null && grep -q 'voice transcript sidecar attached' "$LOG_FILE" 2>/dev/null; then
    echo "Voice transcript sidecar PID=$SIDECAR_PID attached WeChat PID=$WECHAT_PID"
    exit 0
  fi
  sleep 0.25
done
