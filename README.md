...
| 待完整验收 | `memory_embeddings`、`memory_facts`、`memory_summaries` 全量异步回填 |
...
| Embedding | `embedding_service.py` | 可选向量生成、召回与 Reranker；不在实时线程同步回填 |
...
- 快速路由默认模型为 `grok-chat-fast`；最终回复按当前 AI 渠道配置，建议使用
  `grok-4.5`。
...
不必须。可以关闭 `embedding.enabled`，保留全文检索、人物缓存和 Graphiti；
pgvector 表结构仍可供后续云端 Embedding 异步回填。
</details>
